...
language sql stable security invoker set search_path='' as $$
 ...
 where f.deleted_at is null and(f.is_public or f.owner_id=auth.uid()) and coalesce(s.key,'internal')<>'open_food_facts' and(q.normalized='' or f.search_text operator(public.%) q.normalized or f.search_text like '%'||q.normalized||'%' or f.name ilike '%'||replace(q.normalized,'macaroni','mac')||'%')
